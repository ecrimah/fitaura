  ...
  if (loading) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50">
        <i className="ri-loader-4-line animate-spin text-4xl text-sienna-500"></i>
      </div>
    );
  }
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6 md:mb-8">
            <div className="flex items-center gap-3 md:gap-4 w-full md:w-auto">
              <div className="w-12 h-12 md:w-16 md:h-16 flex-shrink-0 rounded-full bg-sienna-50 flex items-center justify-center text-sienna-500 text-xl md:text-2xl font-bold shadow-inner border-2 border-white">
                  ...
                  {[
                    ...
                    <button
                      ...
                      className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl font-medium transition-all text-left group ${activeTab === tab.id
                        ? 'bg-cream-100 text-sienna-500 shadow-sm'
                        : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900'
                        }`}
                    >
                      <i className={`${tab.icon} text-xl transition-colors ${activeTab === tab.id ? 'text-sienna-500' : 'text-gray-400 group-hover:text-gray-600'}`}></i>
                      <span>{tab.label}</span>
                    </button>
                  ))}
                </nav>
                ...
                {[
                  ...
                  <button
                    ...
                    className={`flex items-center gap-2 px-5 py-2.5 rounded-full font-medium whitespace-nowrap transition-all border shadow-sm ${activeTab === tab.id
                      ? 'bg-sienna-500 text-white border-sienna-500 ring-2 ring-sienna-50'
              ...
              <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-4 sm:p-8 min-h-[500px]">
                {activeTab === 'profile' && (
                  <div className="max-w-2xl">
                    ...
                    {profileMessage.text && (
                      <div className={`mb-6 p-4 rounded-xl flex items-start gap-3 ${profileMessage.type === 'success' ? 'bg-cream-100 text-sienna-500 border border-sienna-50' : 'bg-red-50 text-red-700 border border-red-100'}`}>
                    ...
                    <form onSubmit={handleUpdateProfile} className="space-y-6">
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div className="space-y-2">
                          ...
                          <input
                            ...
                            className="w-full px-4 py-3 border-2 border-gray-200 rounded-xl focus:ring-4 focus:ring-cream-100 focus:border-sienna-500 transition-all bg-gray-50 focus:bg-white"
                          />
                        </div>
                        <div className="space-y-2">
                      ...
                      <div className="space-y-2">
                        ...
                        <div className="relative">
                          ...
                          <input
                            ...
                            className="w-full pl-11 pr-4 py-3 border-2 border-gray-200 rounded-xl focus:ring-4 focus:ring-cream-100 focus:border-sienna-500 transition-all bg-gray-50 focus:bg-white"
                          />
                        </div>
                      </div>
                      ...
                      <div className="pt-4">
                        <button
                          ...
                          className="px-8 py-3 bg-sienna-500 hover:bg-sienna-600 text-white rounded-xl font-semibold transition-all shadow-lg shadow-sienna-500/20 active:scale-95 disabled:opacity-50 disabled:shadow-none"
                        >
                    ...
                    <div className="mt-12 pt-12 border-t border-gray-100">
                      ...
                      {passwordMessage.text && (
                        <div className={`mb-6 p-4 rounded-xl flex items-start gap-3 ${passwordMessage.type === 'success' ? 'bg-cream-100 text-sienna-500 border border-sienna-50' : 'bg-red-50 text-red-700 border border-red-100'}`}>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      {securityOptions.map((option, index) => (
                        <Link
                          key={index}
                          href={option.link}
                          className="flex items-center justify-between p-5 border border-gray-200 rounded-2xl hover:border-sienna-500 hover:shadow-md transition-all group bg-white"
                        >
                          <div className="flex items-center gap-3 min-w-0">
                            <div className="w-12 h-12 bg-gray-50 rounded-full flex items-center justify-center group-hover:bg-sienna-50 group-hover:text-sienna-500 transition-colors flex-shrink-0">
                          ...
                          <div className="flex items-center gap-3 flex-shrink-0">
                            {option.status === 'verified' && (
                              <span className="text-[10px] sm:text-xs font-bold px-2 sm:px-3 py-1 bg-sienna-50 text-sienna-500 rounded-full flex items-center gap-1">
                            ...
                            <i className="ri-arrow-right-line text-gray-300 group-hover:text-sienna-500 transition-colors"></i>
                          </div>
                        </Link>
                      ))}
                    </div>
