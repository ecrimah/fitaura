...
export default function OfflinePage() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-cream-100 via-white to-cream-100 flex items-center justify-center px-6">
      <div className="max-w-md w-full text-center">
        {/* Animated illustration */}
        <div className="relative mb-8">
          <div className="w-32 h-32 mx-auto relative">
            {/* Outer ring */}
            <div className="absolute inset-0 rounded-full border-4 border-sienna-50 animate-pulse" />
            {/* Inner circle */}
            <div className="absolute inset-3 rounded-full bg-gradient-to-br from-cream-100 to-sienna-50 flex items-center justify-center">
              <div className="relative">
                <i className="ri-wifi-off-line text-5xl text-sienna-500" />
                {/* Animated dots */}
        ...
        <div className="space-y-3 mb-8">
          <button
            onClick={handleRetry}
            disabled={retrying}
            className="w-full bg-sienna-500 hover:bg-sienna-600 disabled:bg-sienna-400 text-white py-4 px-6 rounded-2xl font-semibold transition-all active:scale-[0.98] shadow-lg shadow-sienna-500/20 flex items-center justify-center gap-2"
          >
            {retrying ? (
              <>
        ...
        <div className="bg-white rounded-2xl p-5 shadow-sm border border-gray-100">
          ...
          <div className="grid grid-cols-2 gap-3">
            {[
              ...
              >
                <i className={`${tip.icon} text-sienna-500 text-sm`} />
                <span>{tip.text}</span>
              </div>
            ))}
          </div>
        ...
        <div className="mt-6 flex items-center justify-center gap-2">
          <div className={`w-2 h-2 rounded-full ${isOnline ? 'bg-sienna-500' : 'bg-red-400'} ${!isOnline && 'animate-pulse'}`} />
