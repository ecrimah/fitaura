...
// JSON-LD for Product + Breadcrumbs is emitted server-side from
// app/(store)/product/[slug]/page.tsx so crawlers see it without executing JS.
    ...
    return (
      <div className="min-h-screen bg-white py-12 flex justify-center items-center">
        <div className="text-center">
          <i className="ri-loader-4-line text-4xl text-sienna-500 animate-spin mb-4 block"></i>
    ...
    return (
      <div className="min-h-screen bg-white py-20 flex justify-center items-center">
        <div className="text-center">
          ...
          <Link href="/shop" className="text-sienna-500 hover:underline">Return to Shop</Link>
        </div>
      </div>
    );
  }
        ...
        <section className="py-8 bg-gray-50 border-b border-gray-200">
          <div className="max-w-7xl mx-auto px-4 sm:px-6">
            <nav className="flex items-center space-x-2 text-sm flex-wrap gap-y-2">
              ...
              <Link href="/shop" className="text-gray-600 hover:text-sienna-500 transition-colors">Shop</Link>
              <i className="ri-arrow-right-s-line text-gray-400"></i>
              <Link href="#" className="text-gray-600 hover:text-sienna-500 transition-colors">{product.category}</Link>
              <i className="ri-arrow-right-s-line text-gray-400"></i>
            ...
            <div className="grid lg:grid-cols-2 gap-12">
                  ...
                  <div className="grid grid-cols-4 gap-4">
                    {product.images.map((image: string, index: number) => (
                      <button
                        ...
                        className={`relative aspect-square rounded-lg overflow-hidden border-2 transition-all cursor-pointer ${selectedImage === index ? 'border-sienna-500 shadow-md' : 'border-gray-200 hover:border-gray-300'
                          }`}
                      >
                        <Image
                          src={image}
              ...
              <div>
                <div className="flex items-start justify-between mb-4">
                  <div>
                    <p className="text-sm text-sienna-500 font-semibold mb-2">{product.category}</p>
                    <h1 className="text-3xl lg:text-4xl font-bold text-gray-900 mb-3">{product.name}</h1>
                  </div>
                  <button
                    onClick={() => setIsWishlisted(!isWishlisted)}
                    className="w-12 h-12 flex items-center justify-center border-2 border-gray-200 hover:border-sienna-500 rounded-full transition-colors cursor-pointer"
                  >
                ...
                {hasVariants && product.colors.length > 0 && (
                  <div className="mb-6">
                    <label className="block font-semibold text-gray-900 mb-3">
                      Color: {selectedColor ? (
                        <span className="text-sienna-500 font-normal">{selectedColor}</span>
                      ) : (
                        ...
                        return (
                          <button
                            ...
                            className={`px-5 py-2.5 rounded-full border-2 font-medium transition-all whitespace-nowrap cursor-pointer flex items-center gap-2 ${isSelected
                              ? 'border-sienna-500 bg-cream-100 text-sienna-500 shadow-sm'
                              : isOutOfStock
                      ...
                      <div className="mb-8">
                        <label className="block font-semibold text-gray-900 mb-3">
                          Variant: {selectedVariant ? (
                            <span className="text-sienna-500 font-normal">{selectedVariant.name} — ${selectedVariant.price?.toFixed(2)}</span>
                          ) : (
                          ...
                          {product.variants.map((variant: any) => {
                            ...
                            return (
                              ...
                              >
                                ...
                                <span className={`text-xs mt-0.5 ${isSelected ? 'text-sienna-500' : 'text-gray-500'}`}>
                ...
                <div className="mb-8">
                  ...
                  <div className="flex items-center space-x-4">
                    ...
                    <div className="flex flex-col">
                      {product.moq > 1 && (
                        <span className="text-sienna-500 font-medium text-sm">
                      ...
                      {activeStock > 10 && (
                        <span className="text-gray-600 font-medium text-sm">
                          <i className="ri-checkbox-circle-line mr-1 text-sienna-500"></i>
                ...
                <div className="flex flex-col sm:flex-row gap-4 mb-8">
                  <button
                    ...
                    className={`flex-1 bg-sienna-500 hover:bg-sienna-600 text-white py-4 rounded-lg font-semibold transition-colors flex items-center justify-center space-x-2 text-lg whitespace-nowrap cursor-pointer ${(activeStock === 0 || needsVariantSelection || needsColorSelection) ? 'opacity-50 cursor-not-allowed' : ''}`}
                  ...
                  {activeStock > 0 && !needsVariantSelection && !needsColorSelection && (
                    <button
                      ...
                      className="sm:w-auto bg-sienna-500 hover:bg-sienna-600 text-white px-8 py-4 rounded-lg font-semibold transition-colors whitespace-nowrap cursor-pointer"
                    >
                      Buy Now
                    </button>
                  )}
                ...
                <div className="border-t border-gray-200 pt-6 space-y-4">
                  <div className="flex items-center text-gray-700">
                    <i className="ri-store-2-line text-xl text-sienna-500 mr-3"></i>
                    <span>Free store pickup available</span>
                  </div>
                  <div className="flex items-center text-gray-700">
                    <i className="ri-arrow-left-right-line text-xl text-sienna-500 mr-3"></i>
                    <span>30-day easy returns and exchanges</span>
                  </div>
                  <div className="flex items-center text-gray-700">
                    <i className="ri-shield-check-line text-xl text-sienna-500 mr-3"></i>
                    <span>Secure payment & buyer protection</span>
                  </div>
                  {product.sku && (
                    <div className="flex items-center text-gray-700">
                      <i className="ri-barcode-line text-xl text-sienna-500 mr-3"></i>
                ...
                {['description', 'features', 'care', 'reviews'].map((tab) => (
                  <button
                    ...
                    className={`pb-4 font-semibold transition-colors relative whitespace-nowrap cursor-pointer ${activeTab === tab
                      ? 'text-sienna-500 border-b-2 border-sienna-500'
                ...
                <ul className="grid md:grid-cols-2 gap-4">
                  {product.features.map((feature: string, index: number) => (
                    <li key={index} className="flex items-start">
                      <i className="ri-checkbox-circle-fill text-sienna-500 text-xl mr-3 mt-1"></i>
