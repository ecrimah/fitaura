  ...
  return (
    <>
      ...
      <main className="min-h-screen bg-gray-50">
        <div className="bg-gradient-to-br from-sienna-500 to-ink-900 text-white py-16">
          <div className="max-w-4xl mx-auto px-4 text-center">
            <h1 className="text-4xl font-bold mb-4">How can we help you?</h1>
            <p className="text-sienna-50 mb-8 text-lg">Search our help center or browse by category</p>
            ...
            <div className="relative max-w-2xl mx-auto">
              <input
                ...
                className="w-full px-6 py-4 pl-14 rounded-xl text-gray-900 text-lg focus:outline-none focus:ring-4 focus:ring-sienna-300"
              />
        ...
        <div className="max-w-7xl mx-auto px-4 py-12">
          <div className="mb-8 flex items-center justify-between">
            ...
            {selectedCategory && (
              <button
                ...
                className="text-sienna-500 hover:text-ink-900 font-semibold whitespace-nowrap"
              >
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6 mb-12">
            {filteredCategories.map((category) => (
              <button
                ...
                className="bg-white rounded-xl shadow-sm p-6 text-left hover:shadow-lg transition-all border-2 border-transparent hover:border-sienna-500"
              >
                <div className="w-14 h-14 flex items-center justify-center bg-sienna-50 rounded-xl mb-4">
                  <i className={`${category.icon} text-3xl text-sienna-500`}></i>
                </div>
                  ...
                  <div className="mt-6 pt-6 border-t border-gray-200 space-y-3">
                    {category.articles.map((article) => (
                      <Link
                        key={article.id}
                        href={`/help/article/${article.id}`}
                        className="block text-sm text-gray-700 hover:text-sienna-500 font-medium"
                      >
                        • {article.title}
                      </Link>
                    ))}
              ...
              {popularArticles.map((article, index) => (
                ...
                >
                  <div className="flex items-center space-x-4">
                    <div className="w-10 h-10 flex items-center justify-center bg-sienna-50 text-sienna-500 rounded-full font-bold">
                      {index + 1}
                    </div>
                    <div>
                      <p className="font-semibold text-gray-900">{article.title}</p>
          ...
          <div className="grid md:grid-cols-3 gap-6">
            ...
            >
              <div className="w-16 h-16 flex items-center justify-center bg-sienna-50 rounded-full mx-auto mb-4">
                <i className="ri-customer-service-2-line text-3xl text-sienna-500"></i>
              </div>
              ...
              <span className="text-sienna-500 font-semibold whitespace-nowrap">Create Ticket →</span>
            </Link>
