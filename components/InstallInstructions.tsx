  ...
  return (
    <>
      <button
        ...
        className="flex items-center gap-2 px-4 py-2 bg-sienna-50 text-sienna-500 rounded-lg hover:bg-sienna-100 transition-colors whitespace-nowrap"
      >
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center z-[10000] p-4">
          <div className="bg-white rounded-2xl max-w-md w-full p-6 animate-scale-in">
            ...
            <div className="space-y-4 mb-6">
              {instructions.steps.map((step, index) => (
                <div key={index} className="flex items-start gap-4">
                  <div className="w-10 h-10 bg-sienna-50 rounded-full flex items-center justify-center flex-shrink-0">
                    <i className={`${step.icon} text-sienna-500`}></i>
                  </div>
                  <div className="flex-1 pt-2">
                    <div className="flex items-center gap-2 mb-1">
                      <span className="w-6 h-6 bg-sienna-500 text-white rounded-full flex items-center justify-center text-xs font-bold">
                        {index + 1}
                      </span>
            ...
            <button
              onClick={() => setShowModal(false)}
              className="w-full bg-sienna-500 hover:bg-sienna-600 text-white py-3 rounded-lg font-semibold transition-colors whitespace-nowrap"
            >
              Got It!
            </button>
          </div>
